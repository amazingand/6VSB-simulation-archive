...
#
# 恒速拉动 S1尾↔S2头 COM 距离 R0 → RF(jar=1, rk2=7.2)。R0/RF 从 smd.RST
#   读取(r2 = 起点, r4 = 终点), 因此各体系可按其平衡 COM 距离自定义起点:
#   直接改 smd.RST 的 r2/r3 即可, 脚本自动重算段数与段目标。
...
# 前置: 当前目录含 step3_input.parm7, eq4_npt.rst7, smd.RST, smd_pull_cleaved.mdin
# ============================================================================
set -euo pipefail
...
# ---------- 从 smd.RST 读取反应坐标起止(单一事实来源) ----------
# r2 = 起点(R0), r4 = 终点(RF); 各体系按平衡 COM 距离改 r2 即自定义起点。
R0="$(python3 - <<'PYEOF'
import re
s = open('smd.RST').read()
m = re.search(r'\br2\s*=\s*([0-9.]+)', s)
if not m: raise SystemExit('错误: smd.RST 未找到 r2(起点)')
print(m.group(1))
PYEOF
)"
RF="$(python3 - <<'PYEOF'
...
if not m: raise SystemExit('错误: smd.RST 未找到 r4(终点)')
print(m.group(1))
PYEOF
)"
echo "==> smd.RST: 反应坐标 R0=${R0} Å → RF=${RF} Å"
...
PYEOF
  ...
  # 生成段 mdin + RST
  python3 - smd_pull_cleaved.mdin smd.RST "smd_chunk${NN}.mdin" "smd_chunk${NN}.RST" "$NST" "$NN" "$R2S" "$R2E" <<'PYEOF'
import sys, re
