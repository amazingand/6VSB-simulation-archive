...
#   例: bash run_smd_cleaved.sh --engine=pmemd.cuda
#
# 后台运行(推荐, 输出到日志):
#   nohup bash run_smd_cleaved.sh --engine=pmemd.cuda > smd_run.log 2>&1 &
#   tail -f smd_run.log
#   # 或 screen: screen -dmS smd_c bash run_smd_cleaved.sh --engine=pmemd.cuda
#
...
while [ $i -lt ${#ARGS[@]} ]; do
  a="${ARGS[$i]}"
  case "$a" in
    cleaved|uncleaved)  [ "$a" = "$SYS" ] || { echo "错误: 本脚本固定用于 cleaved, 收到体系 '$a'"; exit 1; } ;;
