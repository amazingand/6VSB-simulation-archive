#!/bin/bash
# ============================================================================
# run_smd_uncleaved_v03.sh — 项目 C' M3 SMD 生产(分片可重启版)
#
# 恒速拉动 S1尾↔S2头 COM 距离 R0 → RF(jar=1, rk2=7.2)。R0/RF 从 smd_uncleaved_v02.RST
#   读取(r2 = 起点, r4 = 终点), 因此各体系可按其平衡 COM 距离自定义起点:
#   直接改 smd_uncleaved_v02.RST 的 r2/r3 即可, 脚本自动重算段数与段目标。
...
# 全部完成(或跳过)后自动合并各段 DUMPAVE → smd.out, 供 analyze_smd_uncleaved_v03.py 分析。
#
# 用法: bash run_smd_uncleaved_v03.sh [--smd-speed 1|5] [--chunk 1] [--engine=引擎]
#   例: bash run_smd_uncleaved_v03.sh --engine=pmemd.cuda
#        bash run_smd_uncleaved_v03.sh --smd-speed 1 --engine=pmemd.cuda
#
# 后台运行(推荐, 输出到日志):
#   nohup bash run_smd_uncleaved_v03.sh --engine=pmemd.cuda > smd_run.log 2>&1 &
#   tail -f smd_run.log
#   # 或 screen: screen -dmS smd_c bash run_smd_uncleaved_v03.sh --engine=pmemd.cuda
#
# 引擎选择(优先级): --engine= > $PMEMD_ENGINE > 自动探测(WSL 默认回退 CPU pmemd)
# 前置: 当前目录含 step3_input.parm7, eq4_npt.rst7, smd_uncleaved_v02.RST, smd_pull_uncleaved_v03.mdin
# ============================================================================
set -euo pipefail
...
[ -n "$SYS" ] || { echo "用法: run_smd_uncleaved_v03.sh [--smd-speed 1|5] [--chunk 1] [--engine=引擎]"; exit 1; }
...
[ -f eq4_npt.rst7      ] || { echo "错误: 缺 eq4_npt.rst7: 请先跑 run_equil_uncleaved_v01.sh 完成平衡"; exit 1; }
[ -f smd_uncleaved_v02.RST           ] || { echo "错误: 缺 smd_uncleaved_v02.RST"; exit 1; }
[ -f smd_pull_uncleaved_v03.mdin     ] || { echo "错误: 缺 smd_pull_uncleaved_v03.mdin"; exit 1; }

# ---------- 从 smd_uncleaved_v02.RST 读取反应坐标起止(单一事实来源) ----------
# r2 = 起点(R0), r4 = 终点(RF); 各体系按平衡 COM 距离改 r2 即自定义起点。
R0="$(python3 - <<'PYEOF'
import re
s = open('smd_uncleaved_v02.RST').read()
m = re.search(r'\br2\s*=\s*([0-9.]+)', s)
if not m: raise SystemExit('错误: smd_uncleaved_v02.RST 未找到 r2(起点)')
print(m.group(1))
PYEOF
)"
RF="$(python3 - <<'PYEOF'
...
if not m: raise SystemExit('错误: smd_uncleaved_v02.RST 未找到 r4(终点)')
print(m.group(1))
PYEOF
)"
echo "==> smd_uncleaved_v02.RST: 反应坐标 R0=${R0} Å → RF=${RF} Å"
...
PYEOF
  ...
  # 生成段 mdin + RST
  python3 - smd_pull_uncleaved_v03.mdin smd_uncleaved_v02.RST "smd_chunk${NN}.mdin" "smd_chunk${NN}.RST" "$NST" "$NN" "$R2S" "$R2E" <<'PYEOF'
import sys, re
...
md = md.replace('DISANG=smd_uncleaved_v02.RST', f'DISANG=smd_chunk{nn}.RST')
...
echo "下一步:  python analyze_smd_uncleaved_v03.py smd.out --speed ${SPEED} --plot"
