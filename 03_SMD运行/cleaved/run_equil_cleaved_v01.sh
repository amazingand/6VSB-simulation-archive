#!/bin/bash
# ============================================================================
# run_equil_cleaved_v01.sh — 项目 C' M3 平衡段 · cleaved 体系(跑到 EQ4 停)
#
# 本脚本为体系专属副本(原共享 run_equil.sh 拆分), 固定体系 = cleaved,
# 引用本目录版本化 mdin(align_min_cleaved_v01 / eq*_cleaved_v01)。
#
# 流程: 对齐极小化(确定性, 云端/本地同步分界点)
#        → EQ1(NVT 升温)→ EQ2-EQ4(NPT 阶梯放开 posres)
#   EQ5 为可选稳定性检验, 用 --eq5 打开
#
# 用法:  bash run_equil_cleaved_v01.sh [--eq5] [--engine=引擎]
#   例:  bash run_equil_cleaved_v01.sh                          # CPU pmemd(默认)
#         bash run_equil_cleaved_v01.sh --eq5                   # 加 EQ5 稳定性检验
#         bash run_equil_cleaved_v01.sh --engine=pmemd.cuda     # 显式启用 GPU
#
# 引擎选择(优先级从高到低):
...
#   以及本套件 mdin(align_min_cleaved_v01 / eq1_nvt_cleaved_v01 … eq4_npt_cleaved_v01)
#
# 产物:
#   align_min.rst7   — 与云侧核验的分界结构(确定性)
#   eq4_npt.rst7/.nc — EQ4 末态, SMD 生产的起点
#
# 核验通过后接续:  bash run_smd_cleaved_v03.sh [--engine=引擎]
# ============================================================================
set -euo pipefail
...
$ENGINE -O -i align_min_cleaved_v01.mdin -p step3_input.parm7 -c step3_input.rst7 \
...
echo "  (2) 核验通过后:  bash run_smd_cleaved_v03.sh [--engine=引擎]"
