...
> 分析代码与数据。部署文件用稳定名(无后缀), 其版本变化在这里 + `history/` 记录。
...
| **P2** | 08-14 | ① `&end` 修正(pmemd/pmemd.cuda 标准, 约束块后必须 `&end`); ② 脚本拆两段 `run_equil.sh` / `run_smd.sh`; R0=28.80; SMD 单段 | `history/P2_20260814_and_end修正/`(11 文件/体系, 从 m3_packages 重建) |
| **P3** | 08-14 | ① SMD 分片可重启(1 ns/段, 断点续跑); ② **DUMPFREQ 修复**(缺 `&wt type='DUMPFREQ'` 时 DUMPAVE 恒为空); ③ analyze_smd.py v1; ④ 每段 mdin 由模板 regex 生成 | 展开快照：`history/P3_20260814_分片可重启/`；原传输容器见该目录的 `SOURCE_ARCHIVES.tsv` |
| **P4** | 08-15 | 初筛核验后 3 项修订: ① **R0 按体系平衡距离**(28.28 / 31.27), 脚本从 smd.RST 读 R0/RF; ② **跳过条件收紧**(rst7 + mdout `Final Performance Info:` + 段目标一致); ③ analyze_smd.py v2(段边界判据 + 解离峰只看拉伸正力 + R0 取数据首行) | 展开快照：`history/P4_20260815_核验修订/`；原传输容器见该目录的 `SOURCE_ARCHIVES.tsv` |
...
| 存档文件 | SHA-256 |
|---|---|
| `history/P2_*/cleaved/run_smd.sh`(P2 单段版) | `bbcdb1653a27efcff43aba23382483e3fbd37044f6c279e19ee2d37496a0969c` |
| `history/P2_*/cleaved/smd.RST`(R0=28.80) | `5714b00ba8ec4d0b241d3f028b11dbd19701de7250d4c9df8c1d1124f3c85fe2` |
| P3 cleaved 原传输容器（路径见 `history/P3_*/SOURCE_ARCHIVES.tsv`） | `2d68fc708a32101691e7abddde3ca4c1e65554a7272c7186a9df379bab935363` |
| P3 uncleaved 原传输容器（路径见 `history/P3_*/SOURCE_ARCHIVES.tsv`） | `d338a100947a70bdb3981e38a504fee4fa99bd9ddb8f0f1f2840ecc520318143` |
| P3 核心原传输容器（路径见 `history/P3_*/SOURCE_ARCHIVES.tsv`） | `3d5bdec39c6de7bfbbdeea2d6f1b1b74fd4a9e938b2743ecb949bee20ea677c3` |
| P4 原传输容器（路径见 `history/P4_*/SOURCE_ARCHIVES.tsv`） | `82de76be417b85dc7ffa4b14a5d190f347c16dded323ace376a4c1b63a8c2637` |

**analyze_smd.py 版本归属：**

| 版本 | 状态 | SHA-256 | 存档位置 |
|---|---|---|---|
| v0 | 暂存草稿，非正式交付链 | `63cdce37976d404943901f8a98cea36d0f43613622bc3eb7ecebeebb2bd9a842` | `history/non_delivery/analyze_smd_v0_63cdce37/` |
| v1 | P3 正式版本 | `3c26487e93149acb40cb77de3490560cd16bba7df1cd212056913c8f6c95dad0` | `history/P3_20260814_分片可重启/construct-X/{cleaved,uncleaved}/` |
| v2 | P4 正式版本；P4.1 未改动该脚本 | `aea068465269461d60f39be0434cc432b8ed4a650d58ffd7c96e252589e57fa3` | `history/P4_20260815_核验修订/common/` 与当前 `protocols/` |
...
| M3_本地操作说明_v02 | **v02**(v01 已归档 history/文档旧版/) | 本地部署手册(P4.1 部署单元命名) |
...
1. **部署件稳定名**:protocols/ 与 structures/ 内文件名不挂版本; 变化记在这里 + history/。
2. **报告/数据升 vNN**:同一主题实质修改 → v02, v03, …; 旧版进 `history/文档旧版/`。
...
- 早期 `analyze_smd.py` 已确认是非交付 v0 草稿，存放于 `history/non_delivery/`；
