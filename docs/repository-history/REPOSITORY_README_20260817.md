...
> 本文件是 2026-08-17 仓库框架的历史快照。当前入口见 [`README.md`](../../README.md)。
...
| 文件 | 用途 |
|---|---|
| [`STATE.md`](legacy-framework/STATE.md) | 当前工作阶段、分支、Gate 和下一步 |
| [`TASKS.md`](legacy-framework/TASKS.md) | 主代理与子代理的任务图及完成条件 |
| [`DECISIONS.md`](legacy-framework/DECISIONS.md) | append-only 工程决策记录 |
| [`MANIFEST.md`](legacy-framework/MANIFEST.md) | 当前仓库文件快照与待处理事项 |
| [`ITERATION_HISTORY.md`](legacy-framework/ITERATION_HISTORY.md) | 输入、脚本、RST 与相关产物的版本历史 |
| [`PROVENANCE_MAP.tsv`](legacy-framework/PROVENANCE_MAP.tsv) | 规范文件与 `simulations/` 部署副本的 SHA-256 对应关系 |
...
权威关系是：`history/` 保存历史，`protocols/` 保存当前规范版本，`simulations/` 保存部署副本和产物。三者通过 SHA-256 映射，不使用硬链接、符号链接或双向自动同步。
...
- 已确认不属于正式交付链的草稿进入 `history/non_delivery/`；未来无法确认归属的对象仍须先隔离，不得强行并入既有版本。
