...
foreach ($name in @('docs', 'protocols', 'analysis', 'history')) {
...
$archiveRoot = Join-Path $worktreeResolved 'history'
New-Item -ItemType Directory -Path $archiveRoot | Out-Null

Copy-NewTree (Join-Path $packageResolved 'history\P1_20260813_初始交付_sander兼容版') (Join-Path $archiveRoot 'P1_20260813_初始交付_sander兼容版')
Copy-NewTree (Join-Path $packageResolved 'history\文档旧版') (Join-Path $archiveRoot '文档旧版')

$p2 = Join-Path $archiveRoot 'P2_20260814_and_end修正'
New-Item -ItemType Directory -Path $p2 | Out-Null
Copy-NewFile (Join-Path $packageResolved 'history\P2_20260814_and_end修正\说明.md') (Join-Path $p2 '说明.md')
Copy-NewTree (Join-Path $packageResolved 'history\P2_20260814_and_end修正\cleaved') (Join-Path $p2 'construct-X\cleaved')
Copy-NewTree (Join-Path $packageResolved 'history\P2_20260814_and_end修正\uncleaved') (Join-Path $p2 'construct-X\uncleaved')

$p3 = Join-Path $archiveRoot 'P3_20260814_分片可重启'
New-Item -ItemType Directory -Path $p3 | Out-Null
Copy-NewFile (Join-Path $packageResolved 'history\P3_20260814_分片可重启\说明.md') (Join-Path $p3 '说明.md')
...
Copy-NewFile (Join-Path $packageResolved 'history\P4_20260815_核验修订\说明.md') (Join-Path $p4 '说明.md')
...
$p2Sources = @(
    ...
    [pscustomobject]@{ source_archive = 'Cprime_migration_20260815/history/P2_20260814_and_end修正'; bytes = ''; sha256 = ''; role = 'authoritative_reconstructed_tree' }
)
...
Normalize-CprimeSpelling -Root (Join-Path $worktreeResolved 'history')
