...
$archive = Join-Path $worktree 'archive'
...
foreach ($system in @('cleaved', 'uncleaved')) {
    $sourceSystem = Join-Path $package "archive\P2_20260814_and_end修正\$system"
    foreach ($file in Get-ChildItem -LiteralPath $sourceSystem -File | Sort-Object Name) {
        ...
        $p2Rows.Add([pscustomobject]@{
            ...
            source_member = "archive/P2_20260814_and_end修正/$system/$($file.Name)"
