#!/bin/bash
# ============================================================================
# run_smd_cleaved.sh — 项目 C' M3 SMD 生产(分片可重启版)
#
# 恒速拉动 S1尾↔S2头 COM 距离 R0 → RF(jar=1, rk2=7.2)。R0/RF 从 smd_cleaved.RST
#   读取(r2 = 起点, r4 = 终点), 因此各体系可按其平衡 COM 距离自定义起点:
#   直接改 smd_cleaved.RST 的 r2/r3 即可, 脚本自动重算段数与段目标。
...
#
# 用法: bash run_smd_cleaved.sh [--smd-speed 1|5] [--chunk 1] [--engine=引擎]
#   例: bash run_smd_cleaved.sh cleaved --engine=pmemd.cuda
#        bash run_smd_cleaved.sh uncleaved --smd-speed 1 --engine=pmemd.cuda
#
# 后台运行(推荐, 输出到日志):
#   nohup bash run_smd_cleaved.sh cleaved --engine=pmemd.cuda > smd_run.log 2>&1 &
#   tail -f smd_run.log
#   # 或 screen: screen -dmS smd_c bash run_smd_cleaved.sh cleaved --engine=pmemd.cuda
#
# 引擎选择(优先级): --engine= > $PMEMD_ENGINE > 自动探测(WSL 默认回退 CPU pmemd)
# 前置: 当前目录含 step3_input.parm7, eq4_npt.rst7, smd_cleaved.RST, smd_pull_cleaved.mdin
# ============================================================================
set -euo pipefail
...
[ -n "$SYS" ] || { echo "用法: run_smd_cleaved.sh <cleaved|uncleaved> [--smd-speed 1|5] [--chunk 1] [--engine=引擎]"; exit 1; }
...
[ -f smd_cleaved.RST           ] || { echo "错误: 缺 smd_cleaved.RST"; exit 1; }
[ -f smd_pull_cleaved.mdin     ] || { echo "错误: 缺 smd_pull_cleaved.mdin"; exit 1; }

# ---------- 从 smd_cleaved.RST 读取反应坐标起止(单一事实来源) ----------
# r2 = 起点(R0), r4 = 终点(RF); 各体系按平衡 COM 距离改 r2 即自定义起点。
R0="$(python3 - <<'PYEOF'
import re
s = open('smd_cleaved.RST').read()
m = re.search(r'\br2\s*=\s*([0-9.]+)', s)
if not m: raise SystemExit('错误: smd_cleaved.RST 未找到 r2(起点)')
print(m.group(1))
PYEOF
)"
RF="$(python3 - <<'PYEOF'
...
if not m: raise SystemExit('错误: smd_cleaved.RST 未找到 r4(终点)')
print(m.group(1))
PYEOF
)"
echo "==> smd_cleaved.RST: 反应坐标 R0=${R0} Å → RF=${RF} Å"
...
PYEOF
  ...
  # 生成段 mdin + RST
  python3 - smd_pull_cleaved.mdin smd_cleaved.RST "smd_chunk${NN}.mdin" "smd_chunk${NN}.RST" "$NST" "$NN" "$R2S" "$R2E" <<'PYEOF'
import sys, re
...
md = md.replace('DISANG=smd_cleaved.RST', f'DISANG=smd_chunk{nn}.RST')
