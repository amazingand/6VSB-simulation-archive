#!/bin/bash
# ============================================================================
# run_equil.sh — 项目 C' M3 平衡段(跑到 EQ4 停, 供云端/本地一致性核验)
#
# 流程: 对齐极小化(确定性, 云端/本地同步分界点)
#        → EQ1(NVT 升温)→ EQ2-EQ4(NPT 阶梯放开 posres)
#   EQ5 为可选稳定性检验, 用 --eq5 打开
#
# 用法:  bash run_equil.sh <cleaved|uncleaved> [--eq5] [--engine=引擎]
#   例:  bash run_equil.sh cleaved                        # CPU pmemd(默认)
#         bash run_equil.sh uncleaved --eq5               # 加 EQ5 稳定性检验
#         bash run_equil.sh cleaved --engine=pmemd.cuda   # 显式启用 GPU
#
# 引擎选择(优先级从高到低):
...
#   以及本套件 mdin(align_min / eq1-eq4)
#
# 产物:
#   align_min.rst7   — 与云侧核验的分界结构(确定性)
#   eq4_npt.rst7/.nc — EQ4 末态, SMD 生产的起点
#
# 核验通过后接续:  bash run_smd.sh <cleaved|uncleaved> [--engine=引擎]
# ============================================================================
set -euo pipefail
...
SYS="${1:-}"
if [ -z "$SYS" ]; then
  echo "用法: run_equil.sh <cleaved|uncleaved> [--eq5] [--engine=引擎]"; exit 1
fi
DO_EQ5=0
ENGINE_ARG=""
for a in "${@:2}"; do
...
$ENGINE -O -i align_min.mdin -p step3_input.parm7 -c step3_input.rst7 \
...
echo "  (2) 核验通过后:  bash run_smd.sh $SYS [--engine=引擎]"
