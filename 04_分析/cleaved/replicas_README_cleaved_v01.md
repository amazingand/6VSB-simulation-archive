...
| SMD | 复用 `run_smd_cleaved_v03.sh`(从 RST 读 R0/RF, 分片可重启), 模板已注入本副本 ig(Langevin 噪声流) |
...
```bash
# 在 CHARMM-GUI amber/ 目录内(与 run_equil_cleaved_v01.sh / run_smd_cleaved_v03.sh 同处):
...
- 分析入口:`analyze_smd_cleaved_v03.py smd.out --speed 1 --plot`(每副本单独跑);
  轨迹级接触分析用 `analyze_smd_contacts_cleaved_v02.py`(见分析脚本说明)。
...
| 文件 | 作用 |
|---|---|
| `run_replica_cleaved_v01.sh` | 副本包装器(ig 注入 → 平衡 → cpptraj 重算 R0 → 副本 RST → SMD) |
| `launch_all_replicas_cleaved_v01.sh` | 4 卡并行启动器 |
| `smd_cleaved_v02.RST`(副本目录内) | 副本反应坐标(新 R0, 其余字面与参考一致) |
