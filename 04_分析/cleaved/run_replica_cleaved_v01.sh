#!/bin/bash
# ============================================================================
# run_replica_cleaved_v01.sh — 项目 C' 副本(独立重复)平衡 + SMD 包装器 · cleaved 体系
#
...
#
# 用法: bash run_replica_cleaved_v01.sh <repN> [--engine=引擎] [--smd-speed 1|5]
#   例: bash run_replica_cleaved_v01.sh 1 --engine=pmemd.cuda --smd-speed 1
#
# 4 卡并行(4×4090 工作站, 在 amber/ 目录):
#   nohup bash run_replica_cleaved_v01.sh 1 --engine=pmemd.cuda --gpu 0 > r1.log 2>&1 &
#   nohup bash run_replica_cleaved_v01.sh 2 --engine=pmemd.cuda --gpu 1 > r2.log 2>&1 &
#   nohup bash run_replica_cleaved_v01.sh 3 --engine=pmemd.cuda --gpu 2 > r3.log 2>&1 &
#   nohup bash run_replica_cleaved_v01.sh 4 --engine=pmemd.cuda --gpu 3 > r4.log 2>&1 &
#   (--gpu N 只设 CUDA_VISIBLE_DEVICES=N, 各副本独占一张卡)
#   或直接: bash launch_all_replicas_cleaved_v01.sh 4 --engine=pmemd.cuda --smd-speed 1
#
# 前置(在 CHARMM-GUI amber/ 目录内运行):
#   step3_input.parm7/.rst7/.pdb、dihe.restraint、
#   run_equil_cleaved_v01.sh、run_smd_cleaved_v03.sh、smd_pull_cleaved_v03.mdin、smd_cleaved_v02.RST
#   (即 03_SMD运行/cleaved/ 部署单元全部文件 + 平衡产物; 首次副本可不含平衡产物, 链会重跑)
#
...
[ -n "$REPN" ] || { echo "用法: run_replica_cleaved_v01.sh <repN> [--engine=] [--smd-speed 1|5] [--gpu N]"; exit 1; }
...
for f in step3_input.parm7 step3_input.rst7 dihe.restraint \
         run_equil_cleaved_v01.sh run_smd_cleaved_v03.sh smd_pull_cleaved_v03.mdin smd_cleaved_v02.RST; do
...
cp -f align_min_cleaved_v01.mdin eq1_nvt_cleaved_v01.mdin eq2_npt_cleaved_v01.mdin eq3_npt_cleaved_v01.mdin eq4_npt_cleaved_v01.mdin "$REPDIR/" 2>/dev/null || true
[ -f eq5_npt_cleaved_v01.mdin ] && cp -f eq5_npt_cleaved_v01.mdin "$REPDIR/"
cp -f run_equil_cleaved_v01.sh run_smd_cleaved_v03.sh smd_pull_cleaved_v03.mdin smd_cleaved_v02.RST analyze_smd_cleaved_v03.py "$REPDIR/"

# ---------- 给 eq1–eq4(及 eq5)注入 ig ----------
# 仅改 &cntrl 内的 ig: 已有则替换值, 没有则在 imin 行后插入。
for m in eq1_nvt_cleaved_v01.mdin eq2_npt_cleaved_v01.mdin eq3_npt_cleaved_v01.mdin eq4_npt_cleaved_v01.mdin eq5_npt_cleaved_v01.mdin; do
  [ -f "$REPDIR/$m" ] || continue
...
python3 - "$REPDIR/smd_pull_cleaved_v03.mdin" "$IGSEED" <<'PYEOF'
import sys, re
path, seed = sys.argv[1], sys.argv[2]
s = open(path).read()
...
open(path, 'w').write(head + '&cntrl' + body)
PYEOF
echo "  已注入 ig=$IGSEED: smd_pull_cleaved_v03.mdin(模板)"

# ---------- 平衡链(align_min 确定性共享 → eq1–eq4, 在副本目录内跑) ----------
cd "$REPDIR"
echo "==> 平衡链: align_min → eq1 → eq2 → eq3 → eq4(ig=$IGSEED)"
bash run_equil_cleaved_v01.sh --engine="$ENGINE"
...
python3 - "smd_cleaved_v02.RST" <<'PYEOF'
import sys, re
rst = sys.argv[1]
txt = open(rst).read()
...
echo "==> 副本 eq4 COM 距离 = ${NEWR0} Å(参考 R0 = $(grep -oP 'r2 = \K[0-9.]+' ../smd_cleaved_v02.RST | head -1) Å)"

# ---------- 生成副本 smd_<name>.RST(仅改 r2/r3 起点, 余字面不变) ----------
python3 - "smd_cleaved_v02.RST" "$NEWR0" <<'PYEOF'
import sys, re
...
echo "==> SMD 分片生产: bash run_smd_cleaved_v03.sh --smd-speed ${SPEED} --engine=$ENGINE"
bash run_smd_cleaved_v03.sh --smd-speed "$SPEED" --engine="$ENGINE"

echo
echo "副本完成。产物在 $REPDIR/:"
echo "  eq4_npt.rst7       副本平衡末态(独立起点)"
echo "  smd_cleaved_v02.RST 副本反应坐标(新 R0=${NEWR0} Å)"
echo "  smd_chunk*.nc/out  SMD 分片轨迹 + DUMPAVE"
echo "  smd.out            合并力/功数据(analyze_smd_cleaved_v03.py 直接分析)"
echo "建议: 与主副本一起用 analyze_smd_contacts_cleaved_v02.py 做轨迹级接触分析。"
