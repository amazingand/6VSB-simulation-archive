...
# -*- coding: utf-8 -*-
"""
analyze_smd_cleaved_v03.py — 解析 SMD(DUMPAVE)输出,绘制力-位移 / 拉力-时间 / 做功曲线。
...
run_smd_cleaved_v03.sh 分片运行把各段 smd_chunkNN.out 按序合并成 smd.out(目标位置连续),
本脚本据此由目标位置/拉速反推时间,并把各段功拼接成全程累计功。

时间起点 R0 取数据首行目标位置 r2[0](= 各体系 smd_cleaved_v02.RST 的起始 r2),因此支持
...
用法:
  python analyze_smd_cleaved_v03.py smd.out [--speed 5] [--plot] [--out smd_curves]
...
def parse(path):
    ...
    if not r2:
        raise SystemExit(f"错误: {path} 未解析出数据行。请确认 smd_pull_cleaved_v03.mdin 含 "
...
def main():
    ...
    if abs(dr0) > 1.5:
        print(f"  [提示] 初始弹簧伸长 Δr={dr0:+.2f} Å(力 {f[0]:+.1f} kcal/mol/Å):"
              f" smd_cleaved_v02.RST 起点 r2 与该体系平衡 COM 距离不匹配,"
