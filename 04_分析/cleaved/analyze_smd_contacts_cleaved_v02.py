...
# -*- coding: utf-8 -*-
"""
analyze_smd_contacts_cleaved_v02.py — 项目 C' 轨迹级接触分析(在本地 / 工作站运行)
======================================================================
...
smd_production/)给出 prmtop / step3_input.pdb / smd_cleaved_v02.RST / smd_chunk*.nc。
...
用法:
  python analyze_smd_contacts_cleaved_v02.py --traj "smd_production/smd_chunk*.nc"
         ...
         --windows 28-31.6,31.6-36,36-45,45-55 --out contacts_cleaved
...
def parse_args():
    ...
    p.add_argument("system", nargs="?", default="cleaved", choices=["cleaved"],
                   help="体系(固定 cleaved; 决定默认 RST / 盐桥对 / 窗口)")
...
def main():
    args = parse_args()
    out = args.out or "contacts_cleaved"
    rst = args.rst or "smd_cleaved_v02.RST"
